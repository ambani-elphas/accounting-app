...
The app now uses Spring Data JPA with separate writer and reader datasource properties. By default both point to the same in-memory H2 database for local development, but the `app.datasource.writer.*` and `app.datasource.reader.*` settings can be pointed at a primary database and a read replica in deployed environments. You can inspect the local database at `http://localhost:8080/h2-console` with JDBC URL `jdbc:h2:mem:accountingdb`, username `sa`, and a blank password.
